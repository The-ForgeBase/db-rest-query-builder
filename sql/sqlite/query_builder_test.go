...
func TestSQLiteQueryBuilder_BuildQuery(t *testing.T) {
	tests := []struct {
		name       string
		method     string
		table      string
		...
		},
		{
			...
			wantQuery: "SELECT * FROM users WHERE age = ? AND active = ?",
			wantParams: map[string]interface{}{
				"age":    "25",
				"active": "true",
			},
		},
		{
			...
			method:    "GET",
			table:     "users",
			relations: []string{"name", "email"},
			filters: map[string]string{
				"age": "25",
			},
			wantQuery: "SELECT name, email FROM users WHERE age = ?",
			wantParams: map[string]interface{}{
				"age": "25",
			},
		},
		{
			...
			wantQuery: "INSERT INTO users (name, email) VALUES (?, ?); SELECT * FROM users WHERE id = last_insert_rowid()",
			wantParams: map[string]interface{}{
				"name":  "John Doe",
		...
		},
		{
			...
			wantQuery: "UPDATE users SET name = ?, email = ? WHERE id = ?; SELECT * FROM users WHERE id = ?",
			wantParams: map[string]interface{}{
				"name":  "John Smith",
